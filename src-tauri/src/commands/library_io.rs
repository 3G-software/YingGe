...
pub async fn check_import_library(
    ...
    // Try to parse as single library
    let export_data: ExportedLibrary = serde_json::from_str(&metadata_content)
...
pub async fn import_library(
    ...
    let library = if let Some(existing) = existing_library {
        ...
        // Create new library
        queries::create_library(&pool, &export_data.library.name, &target_path).await?
    };
    ...
    for asset_export in &export_data.assets {
        ...
        let zip_file_path = format!("files/{}", asset_export.relative_path);
        let file_data_opt = {
